    ...
    def _seed_cortes(self):
        ...
        for c in cortes:
            if isinstance(c.get("creado_en"), str):
                c["creado_en"] = timezone.make_aware(
                    dt.strptime(c["creado_en"], "%Y-%m-%d %H:%M:%S")
                )
    ...
    def _seed_historial_estado_pedido(self):
        ...
        for h in historial:
            if isinstance(h.get("creado_en"), str):
                h["creado_en"] = timezone.make_aware(
                    dt.strptime(h["creado_en"], "%Y-%m-%d %H:%M:%S")
                )
    ...
    def _seed_mensajes(self):
        ...
        for m in mensajes:
            if isinstance(m.get("creado_en"), str):
                m["creado_en"] = timezone.make_aware(
                    dt.strptime(m["creado_en"], "%Y-%m-%d %H:%M:%S")
                )
    ...
    def _seed_recibos(self):
        ...
        for r in recibos:
            if isinstance(r.get("creado_en"), str):
                r["creado_en"] = timezone.make_aware(
                    dt.strptime(r["creado_en"], "%Y-%m-%d %H:%M:%S")
                )
