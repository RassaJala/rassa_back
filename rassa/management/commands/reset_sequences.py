        ...
        with connection.cursor() as cursor:
            ...
            for table in tables:
                try:
                    ...
                    # attidentity is '' for legacy serial/bigserial; non-empty for IDENTITY.
                    cursor.execute(
                        """
                ...
                pk_column, identity = row
                q_table = connection.ops.quote_name(table)
                q_pk = connection.ops.quote_name(pk_column)

                if identity:
                    # IDENTITY column (Django 5.x / PostgreSQL 10+): use ALTER TABLE ... RESTART WITH.
                    # RESTART WITH expects a literal integer, so compute MAX + 1 first.
                    cursor.execute(
                        f"SELECT COALESCE((SELECT MAX({q_pk}) FROM {q_table}), 0) + 1",
                    )
                ...
                # Legacy serial / bigserial path: reset the backing sequence.
                ...
                # setval(..., max, true) marks the sequence as called so the next value is max + 1.
                sql = f"SELECT setval({q_seq}, COALESCE((SELECT MAX({q_pk}) FROM {q_table}), 1), true);"
                if dry_run:
                    self.stdout.write(sql)
                else:
                    cursor.execute(sql)
